...
(defun scoped-registry (parent &rest isolated)
  (make-instance 'meow:registry :parent parent :isolated isolated))

(test scoped-registry-holds-only-isolated-names
  (with-fresh-registry (r)
    (let ((scope (scoped-registry r :svc))
          (outer (parked-process)))
      (meow:register :svc outer)
      (meow:register :svc (meow:self) :registry scope)
      (meow:register :other (meow:self) :registry scope)
      (is (eq outer (meow:lookup :svc)))
      (is (eq (meow:self) (meow:lookup :svc :registry scope)))
      (is (eq (meow:self) (meow:lookup :other)) "other names live in the parent")
      (is (null (set-exclusive-or '(:svc :other) (meow:names :registry scope))))
      (is (null (set-exclusive-or '(:svc :other) (meow:names))))
      (is-true (meow:unregister :svc :registry scope))
      (is (eq outer (meow:lookup :svc)))
      (is (null (meow:lookup :svc :registry scope)) "no fallback to the parent")
      (meow:send outer :done)
      (join outer))))

(test scoped-registry-await-and-subscribe
  (with-fresh-registry (r)
    (let ((scope (scoped-registry r :svc)))
      (meow:register :svc (meow:self))
      (is (equal '(nil :timeout)
                 (multiple-value-list
                  (meow:await :svc :timeout 0.1 :registry scope))))
      (meow:subscribe :svc :registry scope)
      (is (null (meow:receive :timeout 0.1)))
      (let ((p (parked-process)))
        (meow:register :svc p :registry scope)
        (is (equal (list :registered :svc p) (meow:receive :timeout 1)))
        (is (eq p (meow:await :svc :timeout 1 :registry scope)))
        (meow:send p :bye)
        (join p)
        (is (equal '(:unregistered :svc :bye) (meow:receive :timeout 1)))))))
